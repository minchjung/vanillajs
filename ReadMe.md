...
# Target for this .. 
### SSR 을 위한 CSR을 설계하는것!  
CSR은 온전히 그 기능을 다하도록 설계를 당연히 해야할 것 이며,  
...
데이터 캐싱만 필요한 클라이언트 요청이라면 api 백엔드 요청으로 처리.   
new html, 즉 SSR이 필요한경우(thru 프록시) 템플릿 구성(by client 로직 활용)후 내려준다.  
우선, 여기까지는 잘 구동되는 편이지만, 정확한 속도 및 메모리 테스트는 진행되지 않았다.;   
but.. 일단 더 구현해보자..  

# After for this..
html 초기 랜더링의 주체가 서버쪽의 SSR 이라면,   
많은 변수에 의해서 템플릿 전체가 다시 구성되어야 할때가 많기 때문에, 뷰 전체를 또 빈번히 구성해야 할것이라는 생각이 든다.    
이는 브라우저를 필요 요소만 랜더링하는 CSR과 비교하면   
브라우저 랜더링 속도 저하 및 UX 불편함도 반드시 존재 할것 같다       
잘 못 생각하기 시작하면, 어느 하나 이 점이 없을것 같다는 생각도 들게 된다. ㅠ          
장점이라 한다면, html에 우리 웹서비스의 내용이 어느정도 채워져서 뷰에 한번 표현이 빠르게 된다는 것과       
그리고 검색 사이트의 크롤링 봇에 키워드로 노출될 가능성이 크 다는것         

그런데 이 두번째 장점이 정말 비지니스적으로 엄청큰 부분일 것이라는 생각이 든다.   
...
검색엔진 봇의 속도를 보면 js 파일이 로딩되는 시간에 이미 수천만개를 훑고 다니게된다.   
하루가 지나고 한달이 지나고 1년만 지난다면,          
검색엔진에 노출될 가능성은 SEO 최적화가 이뤄지지 않은 사이트 대비, 말로 할 수 없을 정도로 차이가 나지 않을까..         
개인적으로 왜..저걸 해야하지 싶은 뉴비뉴비한 생각만 들었지만..      
경제적 관점에서 안하면 정말 안되는 구나 하는 생각이 더 강하게 든다.        
